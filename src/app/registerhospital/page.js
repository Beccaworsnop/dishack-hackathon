  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-cover bg-center" style={{ backgroundImage: "url('/hospital-bg.jpg')" }}>
      <nav className="w-full bg-white py-4 px-10 flex justify-between items-center shadow-md fixed top-0">
        <div className="text-red-600 font-bold text-lg">Guest Mode</div>
      ...
      <div className="mt-10 bg-white p-6 rounded-2xl shadow-lg w-96">
        ...
        <p className="text-center text-gray-600">Already have an account? <a href="/signIn" className="text-red-500 font-bold">Log in</a></p>
        
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          <div className="flex space-x-4">
            <input type="text" name="Hospital" placeholder="Hospital Name" className="w-1/2 p-2 border-b focus:outline-none" onChange={handleChange} />
          ...
          <input type="price" name="card" placeholder="Golden Card Number" className="w-full p-2 border-b focus:outline-none" onChange={handleChange} />
          <Link href="/hospitalpacks"><button type="submit" className="w-full bg-red-600 text-white py-2 rounded-full text-lg font-bold">Create Account</button></Link>
        </form>
      </div>
    </div>
  );
